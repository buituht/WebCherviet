            ...
            for (FileItem item : items) {
                if (item.getFieldName().equals("id")) {
                    ...
                    if (item.getSize() > 0) {
                        ...
                        // Giữ icon cũ nếu không có file mới được tải lên
                        // Bạn cần lấy icon cũ từ database và gán vào đây
                        // Ví dụ: category.setIcon(cateService.get(category.getId()).getIcon());
                    }
                }
            }
